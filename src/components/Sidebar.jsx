...
const Sidebar = ({ isOpen, toggleSidebar, isAuthenticated, reference }) => {
  ...
  const isDashboardRoute = ['/dashboard', '/usage', '/billing', '/bookmarks'].includes(pathname);
  ...
  return (
    <motion.div
      className={`fixed top-0 left-0 min-h-screen sm:w-80 ${
        isAuthenticated ? "w-50" : "w-70"
      } dark:bg-[#050816] bg-white shadow-lg z-50`}
    ...
    >
      <div className="p-4 flex flex-row w-full justify-between">
        ...
        <div className="test-md font-bold dark:text-white text-black hidden sm:block">
          Awesome AI tools
        </div>
        <button
        ...
        >
          <svg
            ...
            strokeWidth={2.5}
            stroke="currentColor"
            className="size-7 text-gray-200 hover:text-gray-400 font-bold transition-colors duration-200"
          >
            <path
              strokeLinecap="round"
      ...
      >
        <div className="flex items-center gap-2 border-gray-600 border-y py-3 px-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
        ...
        <div
          className={`border-b border-gray-600 py-2 flex-col gap-1 mb-2 ${
            isAuthenticated ? "hidden" : "block"
          }`}
          onClick={toggleSidebar}
        >
          <LoginLink className={`items-center gap-2 mb-2 flex px-4`}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
          ...
          <RegisterLink
            className="items-center gap-2 mb-2 flex px-4"
            onClick={toggleSidebar}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {sidebarLists.map((item, index) => (
          <Link
            ...
            href={item.path}
            className={`py-2 px-4 ${
              isDashboardRoute ? "hidden lg:block" : "block"
            }`}
          >
